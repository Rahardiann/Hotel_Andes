...
export default class kamar extends React.Component {
    ...
    _handleFilter = () => {
        ...
        let url = "http://localhost:8080/kamar/find/filter"
        axios.post(url, data)
            .then(response => {
    ...
    handleSave = (e) => {
        ...
        if (this.state.action === "insert") {
            let url = "http://localhost:8080/kamar/add"
            ...
            let url = "http://localhost:8080/kamar/update/" + this.state.id_kamar
    ...
    handleDrop = (id) => {
        let url = "http://localhost:8080/kamar/delete/" + id
        if (window.confirm("Are you sure to delete this kamar")) {
                    ...
                    <div class="main-content flex flex-col flex-grow p-4">
                        ...
                        <div className="flex mt-2 flex-row-reverse mr-4">
                            <div className="flex rounded w-1/2">
                                <input
                                    type="text"
                                    className="w-2/3 b0lock w-full px-4 py-2 bg-white border rounded-md focus:border-blue-400 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40"
                                    placeholder="Search..."
                                    name="keyword"
                                            ...
                                            <tbody className="bg-white divide-y divide-gray-200">
                                                {this.state.kamar.map((item, index) => (
                                                    <tr key={index}>
                                                        ...
                                                        {/* {this.state.tipe_kamar.sort((item)=>( */}
                                                        {/* <td className="px-6 py-4 whitespace-nowrap">
                                                            <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-indigo-100 text-indigo-800"> */}
                                                                
                                                                {/* {this.response.tipe_kamar.nama_tipe_kamar} */}
                                                        ...
                                                        {this.state.role === "admin" &&
                                                            <td className="px-6 py-4 whitespace-nowrap">
                                                                ...
                                                                <button class="bg-red-500 hover:bg-red-700 text-white py-1 px-2 rounded" onClick={() => this.handleDrop(item.id_kamar)}>
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-white w-1/3">
                            ...
                            <div class="px-6 py-6 lg:px-8">
                                ...
                                <form class="space-y-6" onSubmit={(event) => this.handleSave(event)}>
                                    <div>
                                        <label for="nomor_kamar" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-800">Nomor Kamar</label>
